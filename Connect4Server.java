...
public class Connect4Server extends Application 
  ...
  class HandleASession implements Runnable, Connect4Constants {
    ...
    private boolean isWon(String token) {
     
    //Checks if any player connected 4 pieces in a column
     
        for (int i = 0; i < 3; i++){
            for (int j = 0; j < 7; j++){
               if (cell[i][j].equals(cell[i+1][j])){
                   if (cell[i][j].equals(cell[i+2][j])){
                       if (cell[i][j].equals(cell[i+3][j])){
                           if (cell[i][j].equals(token)){
                               return true;
                           }
                       }
                   }
    ...
    // Checks if any player connected 4 pieces in a row
        for (int i = 0; i < 6; i++){
            for (int j = 0; j < 3; j++){
               if (cell[i][j].equals(cell[i][j+1])){
                   if (cell[i][j].equals(cell[i][j+2])){
                       if (cell[i][j].equals(cell[i][j+3])){
                           if (cell[i][j].equals(token)){
                               return true;
                           }
                       }
    ...
    // Checks if any player connected 4 pieces in a diagonal
        ...
        for (int i = 0; i < 3; i++){
            for (int j = 0; j < 4; j++){
                if (cell[i][j].equals(cell[i+1][j+1])){
                    if (cell[i][j].equals(cell[i+2][j+2])){
                        if (cell[i][j].equals(cell[i+3][j+3])){
                            if (cell[i][j].equals(token)){
                                return true;
                            }
                        }
